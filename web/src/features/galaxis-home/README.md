...
6. **价目表的厂商图标**走 `getLobeIcon()`——接口返回的 `vendors[].icon` 本来就是
   lobehub 的图标名（`OpenAI`、`Claude.Color`）。原型是纯静态 HTML、没有图标库，
   那一列在原型里不渲染，所以这张表两边列数不同（原型 5 列 / 这里 6 列）。
   价目表本来就不是机械转换出来的（它要取数、要筛选），两边只需保持
   `pricing.ts` 的**换算逻辑**一致。
